...
- commercial/product safety: code license, weights, datasets, voices, and consent are separate facts;
...
4. Never route research-only, non-commercial, unresolved-license, or restricted checkpoints into the
   product path by default.
5. Never put model weights, large generated audio, secrets, or personal reference audio in Git.
6. External workers stay isolated. Do not solve dependency conflicts by contaminating the Core env.
7. "Runs on CPU" is not the same as "good CPU choice". Use measured evidence when routing.
8. Preserve upstream attribution/notices and record exact source/checkpoint revisions when known.
9. A README statement is not a benchmark result. Upstream marketing numbers and lab measurements
   must remain distinguishable.
10. Do not merge to `main` automatically unless the repository owner explicitly asks. Push tested
...
style presets, pronunciation links, and backend-specific cached states. A VoicePack with only unknown
references must not be silently used for cloning.

Future backend embeddings/states should be generated once and cached when upstream APIs support a
stable exported representation. Do not invent binary formats. Pocket TTS's official exported voice
state is the first verified v0.4 implementation.
...
- VoxCPM2: automatic language recognition, Voice Design prompt, controllable cloning, reference
  transcript for high-fidelity/continuation mode.
...
A stale aggregate workflow must not stay red after a corrected dedicated workflow proves the fixed
path. Replace stale broad PR checks with targeted checks and retained evidence.

Whenever a worker's newly exposed capability changes, add a targeted real-model smoke for that path.
For the v0.4 VoxCPM2 adapter, verify actual Voice Design generation and retain the WAV artifact.
...
- batch manifest and content-addressed cache exist;
- Pocket TTS exported VoicePack state can be created and reused without importing Pocket into Core;
...
- CosyVoice3 registry evidence matches the successful corrected run without falsely making it
  product-routable;
- stale aggregate PR workflow is no longer a permanent red check;
